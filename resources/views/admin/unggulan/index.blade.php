@extends('layouts.app', ['title' => 'Keunggulan - Admin'])
    ...
    <div class="container mx-auto px-6 py-8">
        ...
        <div class="flex items-center">
            <button class="text-white focus:outline-none bg-gray-900 px-4 py-2 shadow-sm rounded-md">
                <a href="{{ route('admin.unggulan.create') }}" class="text-white flex">
            ...
            <div class="relative mx-4">
                ...
                <form action="{{ route('admin.unggulan.index') }}" method="GET">
            ...
            <div class="inline-block min-w-full shadow-sm rounded-lg overflow-hidden">
                <table class="min-w-full table-auto">
                    <thead class="justify-between">
                        <tr class="bg-gray-900 w-full">
                            ...
                            <th class="px-16 py-2 text-left"  style="width: 60%">
                        ...
                        @forelse($unggulans as $unggulan)
                            <tr class="border bg-white">
                                ...
                                <td class="px-16 py-2 flex justify-center">
                                    <img src="{{asset('storage/unggulans/'.$unggulan->image)}}" class="admin-index-image">
                                </td>
                                <td class="px-16 py-2">
                                    <b>{{ $unggulan->title }}</b><br>
                                    {!! $unggulan->description !!}
                                </td>
                                <td class="px-10 py-2 text-center">
                                <div class="flex">
                                        <a href="{{ route('admin.unggulan.edit', $unggulan->id) }}" class="bg-indigo-600 px-2 py-2 rounded shadow-sm text-xs text-white focus:outline-none"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">  <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg></a>
                                        <button onClick="destroy(this.id)" id="{{ $unggulan->id }}" class="bg-red-600 px-2 py-2 mx-2 rounded shadow-sm text-xs text-white focus:outline-none"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">  <path stroke-linecap="round" stroke-linejoin="round" d="M9.75 9.75l4.5 4.5m0-4.5l-4.5 4.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></button>
                                    </div>
                                </td>
                            </tr>
                        @empty
                ...
                @if ($unggulans->hasPages())
                    <div class="bg-white p-3">
                        {{ $unggulans->links('vendor.pagination.tailwind') }}
                    </div>
                @endif
            </div>
        </div>
